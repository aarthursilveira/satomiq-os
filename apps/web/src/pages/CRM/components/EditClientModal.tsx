...
export function EditClientModal({ open, onClose, client }: EditClientModalProps): JSX.Element {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({
    defaultValues: {
      ...
      paymentDay: client.paymentDay != null ? String(client.paymentDay) : "",
    },
  });
    ...
    <Modal open={open} onClose={onClose} title="Editar Cliente" size="md">
      <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col gap-4">
        ...
        <div className="flex gap-2 pt-1 border-t border-border-subtle">
          ...
          <Button type="submit" variant="primary" loading={updateClient.isPending} className="flex-1">
            Salvar
          </Button>
        </div>
      </form>
